<template>
	...
	<view style="" class="flex flex-dc flex-ac">
		...
		<view class=" z-5 bg-f" style="width: 750rpx;height: 88rpx;" :style="'top:' + (statusBarHeight * 2) + 'rpx'">
		</view>
		...
		<view class=" bg-f flex flex-jc"
			...
			<view class=" flex flex-dc" style="width: 650rpx;">
				...
				<up-swipe-action autoClose>
					<up-swipe-action-item @click="deleteGoods" duration="100" :autoClose="true"
						v-for='item in goodsInfoArray' :key='item.id' :name="item.id" v-model:show="item.show"
						:options="options1">
						<view class=" flex flex-dc bor-b2sf0" style="min-height: 220rpx;padding-right: 20rpx;">
							<view class="flex flex-ac " style="height: 220rpx;">
								<radio style="transform:scale(0.7)" borderColor="#d1d1d1" activeBorderColor='#21cc5b'
									:checked="item.status == 1" @tap="item.status = item.status == 1 ? 0 : 1"
									color='#21cc5b'></radio>
								<view class=" flex flex-ac flex-g h-100-">
									<image class="" style="width: 182rpx;height: 182rpx;" :src="item.coverImage"
										mode="">
									</image>
...
function getGoodsInfoArray() {
	...
	for (const element of goodsInfoStorage) {
		element.show = false
	}
	goodsInfoArray.value = goodsInfoStorage

	console.log(goodsInfoArray.value)
}
